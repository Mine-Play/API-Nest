...
export class IncidentsService {
    constructor(@InjectRepository(Incident) private incidentsRepository: Repository<Incident>) {}
    ...
    async register(stackTrace: string, ip: string): Promise<Incident>{
        ...
        const incident = this.incidentsRepository.create({ stackTrace: clearStackTrace, ip, status: IncidentStatus.NEW });
    ...
    async setStatus(incident: Incident, status: IncidentStatus): Promise<Incident> {
