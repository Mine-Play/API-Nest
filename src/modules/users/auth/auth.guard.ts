...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const session = await this.sessionsService.getById(request.headers.session); // Get current session
    ...
    if(payload.exp <= Math.round(new Date().getTime() / 1000)){
      const refreshedToken = await this.jwtService.signAsync({ id: payload.id, session: payload.session });
