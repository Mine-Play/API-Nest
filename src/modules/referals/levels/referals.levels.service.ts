...
export class ReferalsLevelsService {
    ...
    async getFirstLevel(isVip: boolean = false): Promise<Referal_level>{
        let referal = await this.referalLevelsRepository.findOne({ where: { needToUse: 0 } });
        if(!referal) {
            referal = await this.referalLevelsRepository.save(await this.referalLevelsRepository.create({ displayName: "Новичёк", needToUse: 0, percents: 2, isVip: isVip }));
        }
        return referal;
    }
    async getByReferal(referal: Referal, isVip: boolean = false): Promise<Referal_level> {
        const level = await this.referalLevelsRepository.findOne({ order: { needToUse: { direction: "DESC" } }, where: { needToUse: MoreThanOrEqual(referal.used), isVip: isVip } });
        return level;
    }
